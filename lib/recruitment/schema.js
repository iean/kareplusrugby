...
-- Referees. One row per person named on an application.
--
-- Only a SHA-256 hash of the access token is stored, so a leak of this table
-- does not hand anyone a working link into a reference form. used_at makes
-- the token single-use, as the spec requires.
--
-- response holds the completed reference as JSON. flagged is set when the
-- referee reports a safeguarding concern or says they would not re-employ, so
-- the weekly digest and the notification subject line can surface it without
...
CREATE INDEX IF NOT EXISTS referees_app_idx    ON referees (application_id);
CREATE INDEX IF NOT EXISTS referees_status_idx ON referees (status);
CREATE INDEX IF NOT EXISTS referees_token_idx  ON referees (token_hash);
`;
