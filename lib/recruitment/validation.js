 ...
 */
export function validateReferees({ referees = [], applicantEmail = "", jobs = [] }) {
  const errors = {};
  const warnings = [];
  ...
  if (mostRecent?.careRole) {
    ...
    if (employer && !matched) {
      errors.referenceCoverage =
        `Your most recent role at ${mostRecent.employer} was in health or social care, so one of your professional referees must be from there. This is a legal requirement for care roles, not a preference — we cannot employ you without evidence of your conduct in that job.`;
    }
  }
