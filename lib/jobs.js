...
function parseAll() {
  ...
  return pages.map((p) => {
    ...
    return {
      ...
      pay: fm.pay || "",
      // The hourly number that goes into baseSalary for Google. ONLY the
      // standard carer rate is asserted. A role that sets its own free-text
      // `pay` — nursing, where the rate is higher and we do not publish a fixed
      // figure — gets no baseSalary rather than a false one. Advertising the
      // £12.71 carer rate as a nurse's salary would be misleading to applicants
      // and to Google, and is exactly the kind of thing to get wrong.
      baseHourly: fm.pay ? null : site.business.pay.basic_hourly,
      summary: fm.summary || "",
      body: p.content || "",
      datePosted,
      validThrough,
      ...
      // The WHOLE slug, not a truncation. Slugs are unique, so this identifier
      // is too. It used to take the first 24 characters, which was fine for one
      // vacancy but collided the moment several roles shared a slug prefix
      // (all four care-home posts begin "care-home-care-assistant-…"). Google
      // uses identifier to tell postings apart; a shared one risks two real
      // vacancies being deduplicated into one listing.
      identifier: `KPR-${String(p.slug).toUpperCase()}`,
    };
  });
}
 ...
 *
 *  - `baseSalary` is the BASIC hourly rate, £12.71 — not the £14.24 total.
 *    Base salary means base. The extra £1.53 is rolled-up holiday pay, which is
 *    a statutory leave entitlement paid alongside the wage rather than reward
 *    for the hour worked; marking it up as base salary would overstate the rate
 *    to Google and to the applicant. The page shows both figures and explains
 *    the split, so the markup and the page agree. Confirmed by Alif 2026-08-24
 *    along with the four conditions that make rolled-up holiday pay lawful —
 *    see business.pay in config/site.json.
 *
...
export function buildJobPostingSchema(job, baseUrl) {
  ...
  return {
    ...
    // The BASIC rate, and ONLY for roles on the standard carer rate. See the
    // note above on why this is not the £14.24 total, and the note on
    // `baseHourly` in parseAll on why a nursing role emits no baseSalary at all
    // rather than the carer figure. baseSalary is "strongly recommended", not
    // required, so omitting it on a role whose rate we do not publish keeps the
    // posting valid and honest.
    ...(job.baseHourly != null
      ? {
          baseSalary: {
            "@type": "MonetaryAmount",
            currency: site.business.pay.currency,
            value: {
              "@type": "QuantitativeValue",
              value: job.baseHourly,
              unitText: "HOUR",
            },
          },
        }
